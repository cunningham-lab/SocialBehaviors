...
from project_ssms.coupled_transformations.momentum_observation import MomentumObservation, MomentumTransformation
from project_ssms.momentum_utils import filter_traj_by_speed
from project_ssms.utils import k_step_prediction_for_momentum_model
...
observation = MomentumObservation(K=K, D=D, M=0, momentum_lags=momentum_lags,
                                  Df=Df, bounds=bounds)
...
momentum_vecs = MomentumTransformation._compute_momentum_vecs(data[:-1], lags=momentum_lags)
...
x_predict = k_step_prediction_for_momentum_model(model, z, data, momentum_vecs=momentum_vecs)
